...
var AppSidebarNav = function(_Component) {
    ...
    AppSidebarNav.prototype.navType = function navType(item, idx) {
        return item.title ? this.navTitle(item, idx) : item.divider ? this.navDivider(item, idx) : item.label ? this.navLabel(item, idx) : item.children ? this.navItem(item, idx) : this.navItem(item, idx);
    };
    ...
    AppSidebarNav.prototype.navLink = function navLink(item, key, classes) {
        ...
        if (item.children && item.children.length > 0) {
            ...
            var currentView = currentUrl[currentUrl.length - 1];
            if (currentView.indexOf(item.viewName) !== -1) {
                var childUrl = getUrl(item.children, currentView);
                url = childUrl ? childUrl : item.url;
            } else {
                url = item.children[0].url;
            }
        }

        function getUrl(items, currentView) {
            var url = null;
            for (var child of items) {
                if (child.viewName === currentView) {
                    url = child.url;
                } else if (child.children) {
                    var childUrl = getUrl(child.children, currentView);
                    if (childUrl) {
                        return childUrl;
                    }
                }
